...
<body>
		...
		<div class="collapse navbar-collapse" id="navbarTogglerDemo01">
			...
			<form class="form-inline my-2 my-lg-0">
				...
				<button class="btn btn-outline-light my-2 my-sm-0" type="submit"
					style="font-style: oblique; font-weight: bold; font-style: italic;">Search</button>
			</form>
		</div>
	</nav>
	...
	<!-- <form action="send" method="get"> -->
		...
		<table class="table table-striped table-dark">
			...
			<c:forEach items="${dto}" var="mainGod">
				<tr>
					<td>${mainGod.name}</td>
					<td>${mainGod.location}</td>
					<td>${mainGod.mainGod}</td>
					<td>${mainGod.pincode}</td>
					<td>${mainGod.fees}</td>
					<td>${mainGod.openTime}</td>
					<td>${mainGod.closeTime}</td>
				</tr>
			</c:forEach>	
